...
//    })
            ...
            $(".accountconfirmsales").live('click',function(){
                var lastnumber = $(this).attr('id')
                var w = 880;
                var h = screen.height;
                ...
                window.open(site_url+"welcome/detail_confirm_business/"+lastnumber.substr(2),
                "_blank",
             ...
             $(".accountofferingsales").live('click',function(){
                var lastnumber = $(this).attr('id');
                var w = 880;
                var h = screen.height;
                var left = (screen.width/2)-(w/2);
                var top = (screen.height/2)-(h/2);
                //window.open(path+"barcodeprint.php?bar="+barcode,
                window.open(site_url+"welcome/detail_offering/"+lastnumber.substr(2),
                "_blank",
                "toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=no, copyhistory=yes, width="+w+", height="+h+",top="+(top-100)+", left="+left+" ");
                return false;
            })
