...
$(document).ready(function() {
    ...
    $('.content-box ul.content-box-tabs li a').click( // When a tab is clicked...
        function() {
            $.ajax({
                type:"POST",
                url: site_url+"activities/load_data_todolist",
                dataType:'html',
                cache: true,
    ...
    $("form#form_task").submit(function() {
        ...
        if($("#form_task").validationEngine({
            ...
            $.ajax({
                type:"POST",
                url: site_url+"activities/add_task",
                cache: true,
    ...
    $("form#form_edit_task").submit(function() {
        ...
        if($("#form_edit_task").validationEngine({
            ...
            $.ajax({
                type:"POST",
                url: site_url+"activities/edit_task",
                cache: true,
                data:$('#form_edit_task').serialize(),
                dataType:"html",
                success: function(data){
                    window.location = site_url +'activities/task';
                    $("#processing").html('');
                    $("input#subject").val('');
                    $("input#startdate").val('');
                    $("input#duedate").val('');
    ...
    $("form#form_close_task").submit(function() {
        ...
        if($("#form_close_task").validationEngine({
            ...
            $.ajax({
                type:"POST",
                url: site_url+"activities/edit_close_task",
                cache: true,
